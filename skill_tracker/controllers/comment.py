...
from skill_tracker.services.comment_service import CommentService
...
async def get_comments_controller(container: AsyncContainer) -> APIRouter:
    ...
    async def create_task(
        ...
        db_comment = await service.create_comment(comment)
        return db_comment
    ...
    async def update_task(
        ...
        db_comment = await service.update_comment(comment_id, comment_update)
        if not db_comment:
            raise HTTPException(status_code=status.HTTP_404_NOT_FOUND, detail="comment not found")
        return db_comment
